...
return -0.5 * dot(conic, vec3(x * x, 2.0 * x * y, y * y));
  ...
  `)}const Jt=""+(se-1),Zt=`${Math.log2(se)}`,ka=""+(ye-1),Ha=`${Math.log2(ye)}`;let fi=class extends Tt{constructor(){super(...arguments),this.tileCameraPosition=O(),this.cameraDelta=O()}};function mi(n){n.code.add(x`float unpackOpacity(uvec4 packedGaussian) {
